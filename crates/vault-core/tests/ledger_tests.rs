...
fn total_sums_all_amounts_without_float_drift() {
    ...
    vault.add_transaction(70_096, "in").unwrap();
    vault.add_transaction(-50_022, "out").unwrap();
    vault.add_transaction(1, "one cent").unwrap();
...
fn update_transaction_keeps_original_date() {
    let dir = tempfile::tempdir().unwrap();
    let vault = LedgerVault::create(dir.path().join("ledger.vault"), "pw", test_params()).unwrap();
    let id = vault.add_transaction(1_000, "typo").unwrap();
...
fn delete_transaction_removes_it_and_updates_total() {
    let dir = tempfile::tempdir().unwrap();
    let vault = LedgerVault::create(dir.path().join("ledger.vault"), "pw", test_params()).unwrap();
    let id = vault.add_transaction(500, "temp").unwrap();
    vault.add_transaction(100, "keep").unwrap();
...
fn wrong_password_is_rejected() {
    ...
    let db_path = dir.path().join("ledger.vault");
    {
        ...
        vault.add_transaction(100, "x").unwrap();
    }
...
fn export_backup_produces_independently_openable_copy() {
    ...
    vault.add_transaction(12_345, "test").unwrap();
