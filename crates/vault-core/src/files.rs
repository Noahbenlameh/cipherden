...
impl FileVault {
    ...
    /// database, deserialized by a `Shell`) as a `FileVault`. There is no
    /// real file behind this — `db_path()` returns `None`.
    ...
    pub fn export_backup(&self, dest_dir: impl AsRef<Path>) -> Result<PathBuf> {
        let db_path = self.db_path.as_ref().ok_or_else(|| {
            VaultError::InvalidMeta(
                "this file vault is an embedded zone with no backing file to back up directly \
