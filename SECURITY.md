...
2. **Никакого сетевого порта вообще.** Приложение — десктопный Tauri-шелл,
   а не веб-сервер: интерфейс общается с бэкендом через внутренний IPC
   Tauri, который не открывает TCP/UDP-сокет ни на `127.0.0.1`, ни где-либо
   ещё. Слушающего порта попросту нет — ни локального, ни тем более
   внешнего. Если когда-нибудь появится функция, которой реально нужен
   сетевой сокет (например, синхронизация), она обязана быть явно
   opt-in, по умолчанию выключенной и пройти проверку безопасности перед
   слиянием.
